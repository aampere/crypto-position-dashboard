...
def updateTargetChanges():
    for i in range(len(targetchanges)):
        try:
            targetchanges[i].set(
                "{:.3f}".format(float(targets[i].get()) / float(positions[i].get()))
            )
        except ValueError:
...
for i in range(len(pairs)):
    ...
    legendactiveposition = tk.Label(
        outputframe, text="Position", font=("Courier", 12), width=8
    )
    legendactiveprice = tk.Label(
        outputframe, text="Price", font=("Courier", 12), width=8
    )
    legendactivechange = tk.Label(
        outputframe, text="Ratio", font=("Courier", 12), width=6
    )
    paircheckboxes.append(
        tk.Checkbutton(
            inputframe, text=pairs[i], variable=pairchecks[i], command=checkToggled
        )
    )
    ...
    activepairlabels.append(
        tk.Label(
            outputframe, textvariable=activepairs[i], font=("Courier", 12), width=8
        )
    )
    activepositionlabels.append(
        tk.Label(
            outputframe, textvariable=activepositions[i], font=("Courier", 12), width=8
        )
    )
    activepricelabels.append(
        tk.Label(
            outputframe, textvariable=activeprices[i], font=("Courier", 12), width=8
        )
    )
    activechangelabels.append(
        tk.Label(
            outputframe, textvariable=activechanges[i], font=("Courier", 12), width=6
        )
    )
